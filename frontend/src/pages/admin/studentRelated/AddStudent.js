...
//       try {
//         const res = await axios.get(
//           `${process.env.REACT_APP_BASE_URL}/SclassList/${user._id}`
//         );
//         setClasses(res.data);
//       } catch (error) {
//         console.error("Error fetching classes", error);
...
//     const data = new FormData();
//     // All top-level fields except 'parentDetails'
//     Object.keys(form).forEach((key) => {
//       if (key === "photo" && form[key]) {
//         data.append("photo", form[key]);
//       } else if (key === "parentDetails") {
...
//     try {
//       await axios.post(`${process.env.REACT_APP_BASE_URL}/StudentReg`, data);
//       alert("Student added successfully");
//       setSubmitted(true);
//       setForm(initialFormState());
//       // Navigate back to students list after short delay, OR use setSubmitted as present
...
//           <p>✅ Student added successfully.</p>
//           <button onClick={() => {
//             // Optionally, go back to students list:
//             // navigate('/Admin/students');
//             handleNewStudent();
//           }}>
//             Add Another Student
...
import axios from "axios";
import { useNavigate } from 'react-router-dom';
import "./AddStudent.css"; // Optional
...
useEffect(() => {
  ...
  const fetchClasses = async () => {
    try {
      ...
      const res = await axios.get(`http://localhost:5001/SclassList/${user._id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // await axios.post(`${process.env.REACT_APP_BASE_URL}/StudentReg`, data);
      await axios.post('http://localhost:5001/StudentReg', data);
      alert("Student added successfully");
      setSubmitted(true);
      setForm(initialFormState());
      // Now, auto-redirect back to the student list after a short delay:
      setTimeout(() => {
        navigate("/Admin/students");
      }, 1000);
    } catch (err) {
  ...
  return (
    <div className="add-student-form">
      ...
      {submitted ? (
        <div className="submitted-box">
          <p>✅ Student added successfully. Redirecting...</p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} encType="multipart/form-data">
          <div className="form-group">
          ...
          <div className="form-group">
            ...
            <input name="section" placeholder="Section" value={form.section} onChange={handleChange} />
            <input name="roll" className="bright-placeholder" placeholder="Roll Number" value={form.roll} onChange={handleChange} required />
            <input name="academicYear" placeholder="Academic Year" value={form.academicYear} onChange={handleChange} />
            <input name="admissionDate" type="date" value={form.admissionDate} onChange={handleChange} />
            <input name="admissionMode" placeholder="Admission Mode" value={form.admissionMode} onChange={handleChange} />
            <input name="previousSchool" placeholder="Previous School" value={form.previousSchool} onChange={handleChange} />
          ...
          <h3>Login Credentials</h3>
          <div className="form-group">
            <input name="password" className="bright-placeholder" type="password" placeholder="Password" value={form.password} onChange={handleChange} required />
          </div>
          <button type="submit">Add Student</button>
        </form>
      )}
