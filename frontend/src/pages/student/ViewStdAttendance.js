...
//     if (!currentUser?._id) return <div>Loading...</div>;
...
const ViewStdAttendance = () => {
  ...
  // Try all known possible attendance fields!
  useEffect(() => {
    if (userDetails) {
      setSubjectAttendance(
        userDetails.attendance ||
        userDetails.monthlyAttendance ||
        userDetails.attendanceRecords ||
        []
      );
    }
  }, [userDetails]);
  ...
  const renderTableSection = () => (
    <>
                      ...
                      <Box sx={{ margin: 1 }}>
                        ...
                        <Table size="small" aria-label="attendance details">
                          ...
                          <TableBody>
                            {allData?.map((data, i) => {
                              const date = new Date(data.date);
                              const dateString = isNaN(date) ? "Invalid Date" : date.toISOString().substring(0, 10);
                              return (
                                <StyledTableRow key={i}>
      ...
      <div>
        Overall Attendance Percentage: {overallAttendancePercentage && !isNaN(overallAttendancePercentage) ? overallAttendancePercentage.toFixed(2) : "0.00"}%
      </div>
    </>
  );
  ...
  // Here’s the important check: Also check subjectAttendance for items!
  return (
    <>
      {loading ? (
        <div>Loading...</div>
